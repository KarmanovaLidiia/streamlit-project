...
def _load_model(qnum: int) -> CatBoostRegressor:
    path = MODELS_DIR / f"catboost_Q{qnum}.cbm"
...
def _align_to_model_features(model: CatBoostRegressor, X: pd.DataFrame) -> pd.DataFrame:
    ...
    if names:
        ...
        return Z[names].astype(float)
    return X.reindex(sorted(X.columns), axis=1).astype(float)
...
def pipeline_infer(input_csv: Path, output_csv: Path) -> None:
    ...
    print("Вычисляем семантическую близость (ruSBERT)...")
    try:
        feats = add_semantic_similarity(feats)
